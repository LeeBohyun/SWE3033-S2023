...
- Once you measured required metrics, remove all the files in ``/path/to/datadir`` and change the db_bench option in order to change compaction style or to improve space amplification. (``/path/to/datadir`` means ``-db="/home/lbh/rocksdb-data"`` in db_bench option)
